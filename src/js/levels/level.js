class Level {
    ...
    foundExit() {
        this.endWith(() => {
            ...
            G.menu = new Menu(
                pick([
                    nomangle('ПОИСК СЕКРЕТНЫХ ДАННЫХ...'),
                    nomangle('GET http://archive.local/data.zip'),
                    nomangle('GET http://localhost/data.zip'),
                    nomangle('ПРОСМОТР ФАЙЛОВ...'),
                ]),
                hasNextLevel ? pick([
                    nomangle('404 НЕ НАЙДЕНО'),
                    nomangle('ФАЙЛ НЕ НАЙДЕН'),
                    nomangle('НИЧЕГО НЕТ'),
                ]) : nomangle('200 НАЙДЕНО!')
            );
            G.menu.animateIn();
    ...
    wasFound() {
        this.endWith(() => {
            ...
            G.menu = new Menu(
                nomangle('ТЕБЯ ЗАМЕТИЛИ!'),
                nomangle('НАЖМИ [R], ЧТОБЫ ПОВТОРИТЬ'),
