...
class Game {
    ...
    constructor() {
        ...
        G.bandanaSource = {'x': INTRO_PLAYER_POSITION.x, 'y': INTRO_PLAYER_POSITION.y - 10};
    ...
    startAnimation() {
        ...
        setTimeout(() => {
            G.menu = new Menu(
                nomangle('ПРОНИКНИ В БАШНЮ'),
                nomangle('НАЙДИ СЕКРЕТНЫЕ ДАННЫЕ')
            );
            G.menu.dim = false;
            G.menu.animateIn();
    ...
    endAnimation() {
        ...
        G.queuedTweet = nomangle('Я прошёл ') + document.title + nomangle(' за ') + formatTime(G.timer) + nomangle(' на сложности ') + G.difficulty.label + '!';

        G.mainMenu();

        // Replace the title
        G.mainTitle = nomangle('ПОБЕДА');
        G.interTitle = '';
        ...
        const value = nomangle(`Найти секретные данные`);
    ...
    render() {
        ...
        if (G.clock % THUNDER_INTERVAL < 0.3) {
            if (G.clock % 0.1 < 0.05) {
                fs('rgba(120, 220, 200, 0.15)');
                fr(0, 0, CANVAS_WIDTH, CANVAS_HEIGHT);
            }

            R.strokeStyle = '#7ad4c8';
            R.lineWidth = 4;
        ...
        wrap(() => {
            ...
            wrap(() => {
                ...
                // Rooftop rubble and decay
                wrap(() => {
                    translate(0, -CELL_SIZE);
                    fs(ROOFTOP_RUBBLE);
                    fr(0, 0, LEVEL_WIDTH, -CELL_SIZE * 2);
                    fr(-CELL_SIZE, -CELL_SIZE, CELL_SIZE * 2, -CELL_SIZE);
                    fr(LEVEL_WIDTH, -CELL_SIZE * 0.5, CELL_SIZE * 2, -CELL_SIZE * 1.5);
                });
                ...
                [
                    ...
                    R.globalAlpha = (sin(G.clock * PI * 2 / 2) * 0.5 + 0.5) * 0.05 + 0.08;
                ...
                wrap(() => {
                    ...
                    translate(INTRO_PLAYER_POSITION.x, INTRO_PLAYER_POSITION.y);
                    renderCharacter(
                        R,
                        G.clock,
                        PLAYER_BODY,
        ...
        wrap(() => {
            ...
            if (G.clock % 2 < 1.5 && G.mainTitleAlpha == 1) {
                const instructions = [
                    nomangle('НАЖМИ [ПРОБЕЛ], ЧТОБЫ НАЧАТЬ'),
                    DIFFICULTY_INSTRUCTION.toUpperCase(),
                ]
                if (G.queuedTweet) {
                    instructions.unshift(nomangle('НАЖМИ [T], ЧТОБЫ ПОДЕЛИТЬСЯ ВРЕМЕНЕМ'));
                }
        ...
        // HUD
        const hudItems = [
            [nomangle('СЛОЖНОСТЬ:'), G.difficulty.label]
        ];
        ...
        if (G.timer) {
            ...
            hudItems.push([
                nomangle('ВРЕМЯ' ) + (G.wasDifficultyChangedDuringRun ? nomangle(' (НЕДЕЙСТВИТЕЛЬНО):') : ':'),
                formatTime(G.timer)
            ]);
        }

        hudItems.push([
            nomangle('РЕКОРД [') + G.difficulty.label + ']:',
        ...
        fs('#5a9a8a');
        fillText(
            nomangle('Геймпад: ') + (gamepads().length ? nomangle('да') : nomangle('нет')),
